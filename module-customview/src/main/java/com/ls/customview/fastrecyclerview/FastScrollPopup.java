...
public class FastScrollPopup {
    ...
    FastScrollPopup(Resources resources, FastScrollRecyclerView recyclerView) {
        ...
        setTextSize(Utils.toScreenPixels(mRes, 12));
        setBackgroundSize(Utils.toPixels(mRes, 32));
    }
    ...
    public void setBackgroundSize(int size) {
        ...
        mCornerRadius = Utils.toPixels(mRes, 6);
        mRecyclerView.invalidate(mBgBounds);
    }
     ...
     */
    public Rect updateFastScrollerBounds(FastScrollRecyclerView recyclerView, Point thumbPoint, Point offsetPoint) {
        ...
        if (isVisible()) {
            ...
            if (mPosition == FastScroller.PopupPosition.CENTER) {
                ...
                if (Utils.isRtl(mRes)) {
                    ...
                    mBgBounds.right = recyclerView.getWidth() - (recyclerView.getScrollBarWidth());
                ...
                //mBgBounds.top = recyclerView.getPaddingTop() - recyclerView.getPaddingBottom() + thumbOffsetY - bgHeight;
                ...
                //mBgBounds.top = Math.max(recyclerView.getPaddingTop(), Math.min(mBgBounds.top, recyclerView.getPaddingTop() + recyclerView.getHeight() - edgePadding - bgHeight));
                mBgBounds.top = thumbPoint.y + offsetPoint.y;
            }
